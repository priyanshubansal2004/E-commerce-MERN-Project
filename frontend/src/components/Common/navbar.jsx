        ...
        <div className='flex items-center space-x-4'>
          {user && user.role === "admin" && (
            ...
            <button onClick={toggleCartDrawer} className='relative hover:text-black'>
                ...
                {cartItemCount > 0 && (<span className='absolute -top-1 bg-red-500 text-white text-xs rounded-full px-2 py-0.5'>{cartItemCount}</span>)}
            </button>
            {/*Search box */}
            <SearchBar></SearchBar>
            <button className='md:hidden'>
                <HiBars3BottomRight onClick={toggleNavDrawer} className='h-6 w-6 text-gray-700'></HiBars3BottomRight>
            </button>
        </div>
    </nav>
